...
import { IDecoded, ISignToken } from '../Types/token.types';
import { UserRoleEnum } from '../Enums/User/user.enum';
import { ISession } from '../Interfaces/Security/client-info.interface';
...
export class TokenService {
  ...
  async saveJwt(
    ...
    await this.jwtRepository.save({
      userId,
      token: await this.hashService.generateHash({text:token}),
      type,
      jti,
  ...
  async decodeToken(token: string, type: TokenTypeEnum) {
    ...
    if (!(await this.hashService.compareHash({plainText:jwt.token,hashText:token}))) {
