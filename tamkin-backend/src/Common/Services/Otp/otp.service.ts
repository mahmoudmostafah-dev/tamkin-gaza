...
export class OTPService {
  ...
  private async checkRequestExists({ userId, type }: { userId: number; type: OTPTypeEnum }) {
  ...
  async verifyOTP({ userId, code, type }: { userId: number; code: string; type: OTPTypeEnum }) {
    ...
    const isCodeValid = await this.hashingService.compareHash({
      hashText: otp.code,
      plainText: code,
    });
