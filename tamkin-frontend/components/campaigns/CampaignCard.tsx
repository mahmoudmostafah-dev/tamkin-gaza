...
const CampaignCard: React.FC<CampaignCardProps> = ({
  ...
  return (
    <Link href={`/campaigns/${uuid}`} className="block group">
      <Card className="w-full max-w-xl overflow-hidden rounded-xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-800 transition-all hover:border-indigo-200">
        ...
        <div className="p-4 space-y-4">
          <h3 className="font-bold text-base text-gray-900 dark:text-white line-clamp-1 group-hover:text-indigo-600 transition-colors">
            {name}
          </h3>
          ...
          <div className="flex flex-wrap gap-1.5">
            {tags.map((tag, i) => (
              <span
                key={i}
                className="text-[9px] font-bold uppercase tracking-widest text-gray-400 bg-gray-50 dark:bg-gray-900 px-1.5 py-0.5 rounded border border-gray-100 dark:border-gray-800"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="flex items-center justify-between border-t border-gray-50 dark:border-gray-700 pt-3">
            ...
            <div className="flex flex-col items-end">
              ...
              <span
                className={`text-sm font-black ${remaining > 0 ? "text-rose-600" : "text-emerald-600"}`}
              >
